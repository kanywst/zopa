...
| user-defined functions, definition or call | Inline the body. A definition's parameters would resolve against the input rather than bind. |
| `else` fallback branches | The fallback would be dropped, leaving only the first condition. |
...
Four of these are worth reading twice if you have used zopa before v0.4.1. `with`, partial rules, `else` branches and function definitions all **converted successfully** while emitting an AST that answered a different question than the Rego it came from -- a dropped modifier, a discarded key, a lost fallback, a parameter silently reinterpreted as an input field. They now refuse, and each has a conformance fixture built to fail rather than skip if the silent behaviour returns. Converting is not the same as deciding correctly -- `zig build test-conformance` covers that, driving real `opa parse` output through the module and comparing decisions.
