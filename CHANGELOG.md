...
- **`rego2ast.py` silently dropped `with` and partial rules.** Both converted "successfully" into an AST that answered a different question than the Rego it came from. `allow if input.x == 1 with input.y as 2` produced exactly the AST of the same rule without the modifier -- OPA evaluates that body against a rewritten input, zopa against the original. `deny contains "x" if ...` became a plain boolean `deny` rule, losing the collected set entirely and turning definitions OPA unions into definitions zopa reports as conflicting. The converter is documented to bail with `Unsupported` on Rego it cannot express, and the conformance runner records that as SKIP; these two slipped through as supported. Both now refuse. This is the same class of divergence the JSON parser is strict about -- a policy the two engines read differently -- arriving through the converter instead.
